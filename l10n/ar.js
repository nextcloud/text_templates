OC.L10N.register(
    "text_templates",
    {
    "Text templates" : "قوالب نصية",
    "Quickly get text templates in Nextcloud" : "أحصل سريعاً على قوالب نصية في نكست كلاود",
    "This app adds a smart picker provider to quickly access text templates when using Text, Collectives, Talk, Mail\nor any place where the smart picker is integrated." : "هذا التطبيق يضيف مُوفّر لاقط ذكي للوصول السريع للقوالب النصية عند استخدام تطبيقات نكست كلاود: النص، و المجموعات، و المحادثة، و البريد الالكتروني\nأو في أي مكان آخر إذا ما كان اللاقط الذكي مدمجاً.",
    "Delete" : "إزالة",
    "Save" : "حفظ",
    "Undo changes" : "تراجع عن التعديلات",
    "Template name" : "اسم القالب",
    "Template content" : "محتوى القالب",
    "Add a template" : "أضف قالباً",
    "Create" : "إنشاء",
    "Cancel" : "إلغاء",
    "Admin-defined templates" : "قوالب معرفة من قبل المستخدم",
    "No admin text templates" : "لا توجد قوالب نصية معرفة من المستخدم",
    "Add admin templates" : "أضف قالباً معرفاً من المستخدم"
},
"nplurals=6; plural=n==0 ? 0 : n==1 ? 1 : n==2 ? 2 : n%100>=3 && n%100<=10 ? 3 : n%100>=11 && n%100<=99 ? 4 : 5;");
