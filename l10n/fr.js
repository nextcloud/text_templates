OC.L10N.register(
    "text_templates",
    {
    "Text templates" : "Modèles de texte",
    "Quickly get text templates in Nextcloud" : "Obtenir rapidement des modèles de texte dans Nextcloud",
    "This app adds a smart picker provider to quickly access text templates when using Text, Collectives, Talk, Mail\nor any place where the smart picker is integrated." : "Cette application ajoute un fournisseur au sélecteur intelligent pour accéder rapidement aux modèles de texte lors de l'utilisation de Text, Collectives, Talk, Mail\nou à tout autre endroit où le sélecteur intelligent est intégré.",
    "Template {name} created" : "Modèle {name} créé",
    "Failed to create template {name}" : "Échec de la création du modèle {name}",
    "Add a new text template" : "Ajouter un nouveau modèle de texte",
    "Create" : "Créer",
    "Cancel" : "Annuler",
    "Save" : "Enregistrer",
    "Undo changes" : "Abandonner les modifications",
    "Template name" : "Nom du modèle",
    "Template content" : "Contenu du modèle",
    "Delete" : "Supprimer",
    "Text templates are snippets of text that users can quickly insert when they're using Text, Collectives, Talk, Mail or any place where the smart picker is available. Here you can create templates that will be available to all users on this instance." : "Les modèles de texte sont des extraits de texte que les utilisateurs peuvent insérer rapidement lorsqu'ils utilisent Text, Collectives, Talk, Mail ou tout autre endroit où le Smart Picker est disponible. Vous pouvez créer ici des modèles qui seront accessibles à tous les utilisateurs de cette instance.",
    "Text templates are snippets of text that you can quickly insert when you're using Text, Collectives, Talk, Mail or any place where the smart picker is available." : "Les modèles de texte sont des extraits de texte que vous pouvez insérer rapidement lorsque vous utilisez Text, Collectives, Talk, Mail ou tout autre endroit où le Smart Picker est disponible.",
    "Template {name} saved" : "Modèle {name} enregistré",
    "Failed to save template {name}" : "Échec de l'enregistrement du modèle {name}",
    "Template {name} deleted" : "Modèle {name} supprimé",
    "Failed to delete template {name}" : "Échec de la suppression du modèle {name}",
    "Add a template" : "Ajouter un modèle",
    "Admin-defined templates" : "Modèles définis par l'administrateur",
    "Admins can create and manage text templates for everyone." : "Les administrateurs peuvent créer et gérer des modèles de texte pour tous les utilisateurs.",
    "Edit admin templates" : "Modifier les modèles d'administration",
    "Search templates" : "Rechercher des modèles",
    "Failed to get templates" : "Échec de l'obtention des modèles",
    "No template found" : "Aucun modèle trouvé",
    "Add templates" : "Ajouter des modèles"
},
"nplurals=3; plural=(n == 0 || n == 1) ? 0 : n != 0 && n % 1000000 == 0 ? 1 : 2;");
